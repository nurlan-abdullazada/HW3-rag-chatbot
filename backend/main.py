    ...
    try:
        ...
        if is_healthy:
            ...
            return HealthResponse(
                status="unhealthy", message="Bedrock service unavailable"
            )
...
async def chat(message: ChatMessage):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Chat failed: {str(e)}") from None
    ...
    try:
        ...
        def generate():
            ...
            for i, word in enumerate(words):
                chunk = {"response": word + " ", "is_complete": i == len(words) - 1}
                yield f"data: {json.dumps(chunk)}\n\n"

        return StreamingResponse(generate(), media_type="text/plain")
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Streaming chat failed: {str(e)}"
        ) from None
